...
public class APIOfficeVisitController extends APIController {
    ...
    public ResponseEntity createOfficeVisit ( @RequestBody final OfficeVisitForm visitForm ) {
        try {
            ...
            // Create the bill based on the Office Visit's CPT codes and save
            // the bill.
            final List<CPTCode> codes = visit.getCptCodes();
            float total = 0;
            for ( final CPTCode c : codes ) {
                total += c.getCost();
            }

            final Bill b = new Bill();
            b.setOfficeVisit( visit );
            if ( ZonedDateTime.now().minusDays( 60L ).isAfter( visit.getDate() ) ) {
                b.setStatus( BillStatus.BILL_DELINQUENT );
            }
            else {
                b.setStatus( BillStatus.BILL_UNPAID );
            }
            b.setTotal( total );
